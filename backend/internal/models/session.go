...
func NewSessionModel(db *database.DB) *SessionModel {
	return &SessionModel{db: db}
}

func (r *SessionModel) Create(userID int, ipAddress, userAgent string, sessionData *tables.SessionData) (*tables.Session, error) {
...
func (r *SessionModel) GetByID(sessionID string) (*tables.Session, error) {
...
func (r *SessionModel) UpdateActivity(sessionID string) error {
	lastActivity := int(time.Now().Unix())
...
func (r *SessionModel) Delete(sessionID string) error {
...
func (r *SessionModel) DeleteByUserID(userID int) error {
...
func (r *SessionModel) CleanupExpired(maxAge time.Duration) error {
...
func (r *SessionModel) GetSessionData(sessionID string) (*tables.SessionData, error) {
	session, err := r.GetByID(sessionID)
	if err != nil {
		return nil, err
	}

	var sessionData tables.SessionData
...
func (r *SessionModel) IsValid(sessionID string, maxIdleTime time.Duration) (bool, error) {
	session, err := r.GetByID(sessionID)
	if err != nil {
