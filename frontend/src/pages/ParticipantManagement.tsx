...
export function ParticipantManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (participantsRes.error) {
        setState.error = participantsRes.error
      } else if ((participantsRes as any).data) {
        // Handle paginated response
        const participantData = (participantsRes as any).data.data || (participantsRes as any).data
        setState.participants = Array.isArray(participantData) ? participantData : []
      }
      ...
      setState.error = 'Failed to load participants'
    } finally {
  ...
  const handleDeleteParticipant = async (id: string) => {
    ...
    try {
      const response = await apiClient.participants.delete(id)
      
      if (response.error) {
        alert('Failed to delete participant: ' + response.error)
      } else {
        await fetchData()
      }
    } catch (err) {
  ...
  const handleSearch = useCallback((e?: React.FormEvent) => {
    e?.preventDefault()
    // Search is handled reactively by filteredParticipants
  }, [])
  ...
  const filteredParticipants = state.participants.filter(participant => {
    const matchesSearch = (participant.name || '').toLowerCase().includes(searchInput.getValue().toLowerCase()) ||
      (participant.email || '').toLowerCase().includes(searchInput.getValue().toLowerCase())
    const matchesGroup = state.selectedGroup === 'all' || (participant.group_id && participant.group_id.toString() === state.selectedGroup)
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {filteredParticipants.slice(0, 15).map((participant, index) => (
                <TableRow key={participant.id}>
                  <TableCell>{index + 1}</TableCell>
                  <TableCell>
                    <span className="font-mono text-sm">{participant.registration_number || participant.id}</span>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <span className="px-2 py-1 rounded-full text-xs font-medium bg-secondary text-secondary-foreground">
                      {participant.group_id ? 'Assigned' : 'Unassigned'}
                    </span>
                  </TableCell>
